...
namespace QuantConnect.Lean.Engine.DataFeeds
{
	...
	public class SQLiteDataProvider : IDataProvider
	{
		...
		private readonly string _dbPath = Config.Get("sqlite-filepath");
		private readonly string _dataPath = Config.Get("data-folder", "../../../Data/");

		/// <summary>
		/// Initialize a new instance of the <see cref="SQLiteDataProvider"/>
		/// </summary>
		...
		/// If file does not exist, an attempt is made to retrieve it from the database
		/// </summary>
		...
		public Stream Fetch(string key)
		{
			...
			{
				Log.Trace("SQLiteDataProvider.Fetch(): Attempting to get data from QuantConnect.com's data library for symbol({0}), resolution({1}) and date({2}).",
					symbol.Value,
					resolution,
					date.Date.ToShortDateString());

                var retrieveSuccessful = 

				var downloadSuccessful = _api.DownloadData(symbol, resolution, date);

				if (downloadSuccessful)
				{
					Log.Trace("SQLiteDataProvider.Fetch(): Successfully retrieved data for symbol({0}), resolution({1}) and date({2}).",
						symbol.Value,
						resolution,
						date.Date.ToShortDateString());
			...
			Log.Error("SQLiteDataProvider.Fetch(): Unable to remotely retrieve data for path {0}. " +
