...
namespace QuantConnect.Brokerages.Poloniex
{
    public partial class PoloniexBrokerage : BaseWebsocketsBrokerage, IDataQueueHandler
    {
        ...
        /// </summary>
        public ConcurrentDictionary<long, PoloniexFill> FillSplit { get; set; }
        ...
        public PoloniexBrokerage(string wssUrl, IWebSocket websocket, IRestClient restClient, string apiKey, string apiSecret, string passPhrase)
            : base(wssUrl, websocket, restClient, apiKey, apiSecret, Market.Poloniex, "Poloniex")
        {
            FillSplit = new ConcurrentDictionary<long, PoloniexFill>();
        ...
        public override void OnMessage(object sender, MessageReceivedEventArgs e)
        {
            try
            {
                ...
                else if (raw.Type == "error")
                {
                    ...
                    Log.Error("PoloniexBrokerage.OnMessage(): " + error.Message);
                }
                ...
                Log.Trace("PoloniexWebsocketsBrokerage.OnMessage(): " + e.Message);
            }
            catch (Exception ex)
            {
        ...
        private void OrderMatch(string data)
        {
            ...
            (
                ...
                GetFee(cached.First().Value), "Poloniex Match Event"
            );
        ...
        private void OrderDone(string data)
        {
            ...
            (
                ...
                GetFee(cached.First().Value), "Poloniex Fill Event"
            );
